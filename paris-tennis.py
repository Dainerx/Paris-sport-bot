...
def book_and_pay(driver, hour, day, name):
    try:
        ...
        driver.find_element_by_id('head{}{}'.format(name, hour)).click()
        time.sleep(2)
        ...
        if BotMode is not HOLD_MODE:
            ...
            logging.info(
                "Held a reservation at {0}, in {1}, {2} days from today - you have 10 minutes to manually pay".format(hour,
                                                                                                             name,
                                                                                                             day))
            notification.notify(
                title='Paris Sport Bot',
...
def check_arguments(args):
    ...
    if not (str(args.Name).replace(" ", "") in PARIS_TENNIS_COURTS):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-n", "--Name", help="Set tennis court name")
